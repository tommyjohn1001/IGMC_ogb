...
from plnlp.utils import adj_normalization, gcn_normalization
...
def main():
    ...
    log_file_name = "logs/log_" + args.data_name + "_" + str(int(time.time())) + ".txt"
    ...
    if args.data_name == "ogbl-collab":
        ...
        if args.use_valedges_as_input:
            full_edge_index = torch.cat(
                [split_edge["valid"]["edge"].t(), split_edge["train"]["edge"].t()], dim=-1
            )
            full_edge_weight = torch.cat(
                [split_edge["train"]["weight"], split_edge["valid"]["weight"]], dim=-1
            )
            # create adjacency matrix
            ...
            if args.use_coalesce:
                full_edge_index, full_edge_weight = coalesce(
                    full_edge_index, full_edge_weight, num_nodes, num_nodes
                )
            ...
            split_edge["train"]["weight"] = (
                deg_inv_sqrt[full_edge_index[0]]
                * full_edge_weight
                * deg_inv_sqrt[full_edge_index[1]]
            )
    ...
    for run in range(args.runs):
        ...
        for epoch in range(1, 1 + args.epochs):
            if args.random_walk_augment:
                ...
                split_edge["train"]["edge"] = torch.masked_select(pairs, mask.view(-1, 1)).view(
                    -1, 2
                )
                split_edge["train"]["weight"] = torch.masked_select(
                    weights.to(device=mask.device), mask
                )
