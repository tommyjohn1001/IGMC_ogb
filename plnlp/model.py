...
from cerei.layers import GNNModule
    ...
    *_hidden_channels : int
    ...
    def __init__(
        ...
        self.encoder = GNNModule(
            ...
            latent_dim=gnn_hidden_channels,
            num_layers=gnn_num_layers,
            dropout=dropout,
            residual=True,
        ).to(device)
        ...
        # Parameter representing edge type embedding
        self.edge_embd = nn.parameter.Parameter(torch.rand((1, self.input_channels)))
    ...
    def get_edge_type_embds(self, n_edges: int):
        edge_type_embds = self.edge_embd.repeat(n_edges, 1).to(device=self.device)
        # [n_edges, d_hid]

        return edge_type_embds
    ...
    def train(self, data, split_edge, batch_size, neg_sampler_name, num_neg):
        ...
        pos_train_edge, neg_train_edge = pos_train_edge.to(self.device), neg_train_edge.to(
            self.device
        )
        ...
        for perm in DataLoader(range(pos_train_edge.size(0)), batch_size, shuffle=True):
            ...
            edge_indx = data.adj_t


            # [2, n_edges]
            edge_type_embds = self.get_edge_type_embds(len(edge_indx.coo()[-1]))
            # [n_edges, d_hid]
            # FIXME: Replace the following by real PE calculated beforehand
            pe = torch.rand_like(input_feat)
            h, pe = self.encoder(input_feat, edge_type_embds, edge_indx, pe)
    ...
    def test(self, data, split_edge, batch_size, evaluator, eval_metric):
        ...
        pos_valid_edge, neg_valid_edge = pos_valid_edge.to(self.device), neg_valid_edge.to(
            self.device
        )
